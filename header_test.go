...
var testmetadatas = []metadataSpec{
	{
		...
		[]byte{
			...
			0x04, 0x6b, 0x65, 0x79, 0x32, 0x06, 0x76, 0x61, 0x6c, 0x75, 0x65, 0x32, // key2 value2
			0x04, 0x6b, 0x65, 0x79, 0x33, 0x06, 0x76, 0x61, 0x6c, 0x75, 0x65, 0x33, // key3 value3
		},
	},
	{
...
func TestWriteThenReadHeader(t *testing.T) {
	for _, spec := range testmetadatas {
		t.Run(spec.Name, func(t *testing.T) {
			...
			trailingJunk := "trailing junk"
			written, err := buf.Write([]byte(trailingJunk))
			assert.NoError(t, err, "trailing junk should write successfully")
			assert.Equal(t, len(trailingJunk), written, "written bytes should be equal")
